...
def main(argv=sys.argv[1:]):
    ...
    logger.info('Remove bright stars from this segmentation map, using method = {}'.format('usno'))
    seg_gaia = mask_out_stars(segmap, hires_3.image, hires_3.header, 
                              method='usno', bright_lim=config.star.bright_lim)
    ...
    image = final.drawImage(scale=config.DF.pixel_scale / f_magnify, 
                            nx=hires_3.shape[1], 
                            ny=hires_3.shape[0])
    save_to_fits(image.array, '_df_model_{}.fits'.format(f_magnify), header=hires_3.header)
    ...
    if config.fluxmodel.unmask_lowsb:
        ...
        E[np.isinf(image.array)] = 0
        E[np.isinf(E) | np.isnan(E)] = 0
        ...
        im_ratio = E
        im_highres_new = copy.deepcopy(hires_fluxmod.image)
        ...
        sb_lim_cpp = 10**((config.fluxmodel.sb_lim - config.DF.zeropoint)/-2.5 + 2*np.log10(config.DF.pixel_scale * f_magnify) )
        ...
        for obj in objects:
            ...
            if ((np.mean(flux_hires) < sb_lim_cpp) and (np.mean(flux_ratio) < config.fluxmodel.unmask_ratio)):
                im_highres_new_slice[ind] = 0
                print('# removed object {}'.format(obj['index']))
        
        im_highres_new[im_seg_ind] = im_highres_new_slice
        save_to_fits(im_highres_new, '_hires_fluxmode_clean.fits')

        psf = galsim.InterpolatedImage(galsim.Image(kernel_med), 
                                    scale=config.DF.pixel_scale / f_magnify)
        gal = galsim.InterpolatedImage(galsim.Image(im_highres_new), 
                                    scale=config.DF.pixel_scale / f_magnify)
        logger.info('Convolving image, this will be a bit slow @_@ ###')
        final = galsim.Convolve([gal, psf])
        image = final.drawImage(scale=config.DF.pixel_scale / f_magnify, 
