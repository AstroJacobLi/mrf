...
def download_decals_cutout(ra, dec, size, band, layer='dr8-south', pixel_unit=False, 
    ...
    '''Download DECaLS image cutout of a given image. Maximum size is 3000 * 3000 pix.
...
def download_decals_large(ra, dec, band, size=0.7*u.deg, radius=0.5*u.deg, layer='dr8-south', verbose=True,
    ...
    if os.path.isfile('_survey-brick.fits'):
        os.remove('_survey-brick.fits')
    urllib.request.urlretrieve(URL, filename='_survey-brick.fits', data=None)

    # Find nearby bricks 
    bricks_cat = Table.read('_survey-brick.fits', format='fits')
